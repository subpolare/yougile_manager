    ...
    *,
    ...
    async def initialize(message: Message, command: CommandObject, bot: Bot) -> None:
        ...
        try:
            ...
            logger.exception("YouGile project lookup failed for /init chat_id=%s", message.chat.id)
        ...
        if len(matches) != 1:
            if argument.isdecimal() and len(matches) > 1:
                ...
                await message.answer("Проект с таким номером не найден или недоступен для привязки.")
            else:
                await message.answer(
